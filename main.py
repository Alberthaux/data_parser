...
from src.data_cleaning import (
    MaterialParser,
    FuzzyMaterialMatcher,
    normalize_material_name,
    split_category_subcategory,
    clean_text
)
from src.models import Product, Part, Material
...
def build_frequency_map(csv_path: str) -> Dict[str, int]:
    """
    Reads the CSV once to gather all material names, normalizes them,
    ...
    with open(csv_path, mode="r", encoding="utf-8") as f:
        ...
        for row in reader:
            care = row["care_label"]
            # parse
            parsed = MaterialParser.parse_care_label(care)
            for part_dict in parsed["parts"]:
                for material_info in part_dict["materials"]:
                    norm = normalize_material_name(material_info["name"])
...
def parse_product_care_label(
    ...
    care_label: str,
    part_id_start: int,
    fuzzy_matcher: FuzzyMaterialMatcher,
    verbose=False,
) -> Product:
    """
    ...
    and each material storing product_id, part_id, original_material_name, brand, etc.
    """
    product_category, product_sub_category = split_category_subcategory(category)
    parsed = MaterialParser.parse_care_label(care_label, verbose=verbose)
    ...
    for part_dict in parts_data:
        ...
        for material_info in part_dict["materials"]:
            # normalized name
            normalized_name = normalize_material_name(material_info["name"])
            canonical_name = fuzzy_matcher.get_canonical_material(normalized_name)

            mat = Material(
                product_id=product_id,
                part_id=current_part_id,
                name=canonical_name,
                proportion=material_info["mix"],
                brand=material_info.get("brand", ""),
                original_material_name=material_info["original_material_name"],
                made_in_france=material_info.get("made_in_france", False),
                solution_dyed=material_info.get("solution_dyed", False),
                recycled=material_info.get("recycled", False),
            )
...
def load_csv_and_parse(
    csv_path: str, fuzzy_matcher: FuzzyMaterialMatcher, verbose=False
) -> List[Product]:
    """
...
def main(csv_path: str = "data/data.csv"):
    ...
    logging.info(
        f"Pre-populating fuzzy matcher with {len(initial_data)} materials (>= 100 occurrences)"
    )

    # 3) Build fuzzy matcher
    fuzzy_matcher = FuzzyMaterialMatcher(threshold=95, initial_data=initial_data)
        ...
        for part in p.parts:
            for mat in part.materials:
                record = {
                    ...
                    "material_proportion": mat.proportion,
                    "brand": mat.brand,
                    "original_material_name": mat.original_material_name,
                    "made_in_france": mat.made_in_france,
                    "solution_dyed": mat.solution_dyed,
                    "recycled": mat.recycled,
                }
                if  is_not_empty_or_false(record["material_name"]) or any([is_not_empty_or_false(record[k]) for k in METADATA_DICT]):

                    material_data_to_insert.append(record)
    exporter.export_to_sql(material_table_name, material_columns, material_data_to_insert)
